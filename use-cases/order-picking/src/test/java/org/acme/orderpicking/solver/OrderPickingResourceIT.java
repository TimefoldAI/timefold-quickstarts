...
class OrderPickingResourceIT {
    ...
    void solveNative() {
        ...
        String datasetId = given().contentType(ContentType.JSON).body(demoDataJson).when()
                .post("/v1/order-picking-plans").then().statusCode(202).extract().jsonPath().getString("id");
        ...
        await().atMost(Duration.ofMinutes(1)).pollInterval(Duration.ofMillis(500L)).until(() -> TERMINAL_STATUSES
                .contains(get("/v1/order-picking-plans/" + datasetId).jsonPath().getString("metadata.solverStatus")));

        var response = get("/v1/order-picking-plans/" + datasetId).then().extract();
