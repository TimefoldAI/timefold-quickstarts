package org.acme.facilitylocation.integrationtest;
...
class FacilityPlanResourceIT {
    ...
    void solveNative() {
        ...
        String datasetId = given().contentType(ContentType.JSON).body(demoDataJson).when()
                .post("/v1/plans").then().statusCode(202).extract().jsonPath().getString("id");
        ...
        await().atMost(Duration.ofMinutes(1)).pollInterval(Duration.ofMillis(500L)).until(() -> TERMINAL_STATUSES
                .contains(get("/v1/plans/" + datasetId).jsonPath().getString("metadata.solverStatus")));

        var response = get("/v1/plans/" + datasetId).then().extract();
