...
class FacilityLocationConstraintProviderTest {
    ...
    ConstraintVerifier<FacilityLocationConstraintProvider, FacilityLocationProblem> constraintVerifier =
            ConstraintVerifier.build(new FacilityLocationConstraintProvider(), FacilityLocationProblem.class,
                    Consumer.class, Facility.class);

    @Test
    void penalizesCapacityExceededByASingleConsumer() {
        Location location = new Location(1, 1);
        Facility facility = new Facility("1", "Facility 1", location, 0, 20);
        Consumer consumer = new Consumer("1", location, 100);
        consumer.setFacility(facility);

        constraintVerifier.verifyThat(FacilityLocationConstraintProvider::facilityCapacity).given(consumer, facility)
                .penalizesBy(80);
    }
    ...
    void noPenaltyWhenDemandLessThanCapacity() {
        Location location = new Location(1, 1);
        Facility facility = new Facility("1", "Facility 1", location, 0, 100);
        ...
        constraintVerifier.verifyThat(FacilityLocationConstraintProvider::facilityCapacity)
                .given(consumer1, consumer2, consumer3, facility).penalizesBy(0);
    }

    @Test
    void noPenaltyWhenConsumerNotAssigned() {
        Location location = new Location(1, 1);
        Facility facility = new Facility("1", "Facility 1", location, 0, 1);
        Consumer consumer = new Consumer("1", location, 100);
    ...
    void shouldPenalizeSetupCost() {
        long setupCost = 123;
        Location location = new Location(1, 1);
        Facility facility = new Facility("1", "Facility 1", location, setupCost, 100);
        Consumer consumer = new Consumer("1", location, 1);
        consumer.setFacility(facility);

        constraintVerifier.verifyThat(FacilityLocationConstraintProvider::setupCost).given(facility, consumer)
    ...
    void shouldPenalizeDistanceToFacility() {
        ...
        Facility facility = new Facility("1", "Facility 1", facilityLocation, 0, 100);
        ...
        constraintVerifier.verifyThat(FacilityLocationConstraintProvider::distanceFromFacility)
                .given(facility, consumer1, consumer2).penalizesBy((int) (30 * Location.METERS_PER_DEGREE));
    }
}
