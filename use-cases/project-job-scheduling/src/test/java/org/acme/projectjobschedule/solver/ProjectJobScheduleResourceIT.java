...
class ProjectJobScheduleResourceIT {
    ...
    void solveNative() {
        ...
        String datasetId = given().contentType(ContentType.JSON).body(demoDataJson).when()
                .post("/v1/project-job-schedules").then().statusCode(202).extract().jsonPath().getString("id");
        ...
        await().atMost(Duration.ofMinutes(1)).pollInterval(Duration.ofMillis(500L)).until(() -> TERMINAL_STATUSES
                .contains(get("/v1/project-job-schedules/" + datasetId).jsonPath().getString("metadata.solverStatus")));

        var response = get("/v1/project-job-schedules/" + datasetId).then().extract();
