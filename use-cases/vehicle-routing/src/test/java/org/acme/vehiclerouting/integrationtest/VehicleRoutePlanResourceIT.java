package org.acme.vehiclerouting.integrationtest;
...
class VehicleRoutePlanResourceIT {
    ...
    void solveNative() {
        ...
        await().atMost(Duration.ofMinutes(2)).pollInterval(Duration.ofMillis(500L)).until(() -> TERMINAL_STATUSES
                .contains(get("/v1/route-plans/" + datasetId).jsonPath().getString("metadata.solverStatus")));

        var response = get("/v1/route-plans/" + datasetId).then().extract();
